...
import { CityOverview, StationData, ViewType } from '@/lib/types/aeropulse';
import { AQIGauge } from '@/components/aeropulse/AQIGauge';
import { PollutantGrid } from '@/components/aeropulse/PollutantGrid';
import { WeatherStrip } from '@/components/aeropulse/WeatherStrip';
import {
  AlertTriangle,
  ArrowRight,
  TrendingUp,
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-slate-200 shadow-md min-h-[220px] sm:min-h-[250px] flex flex-col justify-between p-6 sm:p-8 transition-all duration-700 bg-slate-900 group">
        ...
        {/* Top Location & Environmental Tag */}
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono uppercase font-black tracking-wider bg-emerald-500/90 text-white px-3 py-1 rounded-full backdrop-blur-md shadow-sm flex items-center gap-1.5">
              ...
              <span>Cubbon Park & Urban Canopy Zone</span>
            </span>
            <span className="text-[11px] font-mono text-slate-200 bg-slate-900/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/20">
              Bengaluru Urban District &bull; CPCB Telemetry
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-white/15 hover:bg-white/25 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/30 text-xs font-bold text-white transition flex items-center gap-1.5"
            >
      ...
      {overview.active_warnings_count > 0 && (
        ...
        >
          ...
          <button 
            ...
            className="flex items-center gap-1 text-xs font-bold text-sky-700 hover:text-sky-800 shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 classy-card rounded-3xl p-6">
          ...
          {/* 24-Hour Backward Trajectory & Surface Meteorology Dispersion */}
          <div className="mt-4 p-4 rounded-2xl bg-gradient-to-r from-slate-900 to-sky-950 text-white space-y-2 text-xs border border-sky-800/40 shadow-sm">
            <div className="flex items-center justify-between border-b border-white/10 pb-2">
              ...
              <span className="text-[10px] font-mono font-bold bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-400/30">
                NOAA HYSPLIT Calibrated
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 font-mono text-[11px] pt-1">
              <div><span className="text-slate-400 text-[10px] block">Airflow Vector</span><b>14.2 km/h (ESE &rarr; WNW)</b></div>
              <div><span className="text-slate-400 text-[10px] block">Boundary Layer Height</span><b>850m &rarr; 180m (Nocturnal Drop)</b></div>
              <div><span className="text-slate-400 text-[10px] block">Primary Source Transport</span><b className="text-amber-300">Hosur Freight & Silk Board Merge</b></div>
            </div>
            <p className="text-[11px] text-slate-300 font-sans leading-snug pt-1 border-t border-white/10">
              Low boundary layer during early morning hours compresses vehicular NO₂ & PM2.5 within 200m of ground surface, causing temporary AQI spikes before thermal dissipation at 11:00 AM.
            </p>
          </div>
        </div>
