...
import { fetchGreenSpaces } from '@/lib/services/aeropulseApi';
...
export const GreenCanopyParksView: React.FC = () => {
  ...
  const getParkImage = (name: string) => {
    ...
    if (n.includes('cubbon')) return '/images/cubbon_park.jpg';
    if (n.includes('lalbagh')) return '/images/lalbagh.jpg';
    if (n.includes('turahalli')) return 'https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&w=600&q=80';
    if (n.includes('bannerghatta')) return 'https://images.unsplash.com/photo-1511497584788-876761c14400?auto=format&fit=crop&w=600&q=80';
    if (n.includes('gkvk')) return 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&w=600&q=80';
    if (n.includes('sankey')) return 'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=600&q=80';
  ...
  const getPlantImage = (name: string) => {
    const n = name.toLowerCase();
    if (n.includes('snake') || n.includes('sansevieria')) return 'https://images.unsplash.com/photo-1593482892290-f54927ae1bf6?auto=format&fit=crop&w=600&q=80';
    if (n.includes('areca') || n.includes('palm')) return 'https://images.unsplash.com/photo-1614594975525-e45190c55d0b?auto=format&fit=crop&w=600&q=80';
    if (n.includes('peace lily')) return 'https://images.unsplash.com/photo-1593691509543-c55fb32e7355?auto=format&fit=crop&w=600&q=80';
    if (n.includes('money') || n.includes('pothos')) return 'https://images.unsplash.com/photo-1617173944883-6ffbd35d584d?auto=format&fit=crop&w=600&q=80';
    if (n.includes('spider')) return 'https://images.unsplash.com/photo-1572688484438-313a6e50c333?auto=format&fit=crop&w=600&q=80';
    if (n.includes('fern')) return 'https://images.unsplash.com/photo-1596547609652-9cf5d8d76921?auto=format&fit=crop&w=600&q=80';
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {plants.map((plant, idx) => {
              const plantImg = getPlantImage(plant.name);
              return (
                <div
                  key={idx}
                  className="classy-card rounded-3xl p-5 border border-slate-200 flex flex-col justify-between space-y-3.5 shadow-sm transition classy-card-hover overflow-hidden"
                >
                  {plantImg && (
                    <div className="h-36 -mx-5 -mt-5 mb-2 overflow-hidden relative">
                      <img src={plantImg} alt={plant.name} className="w-full h-full object-cover" />
                      <div className="absolute top-2.5 right-2.5">
                        <span className="text-xs font-mono font-black text-emerald-900 bg-white/90 backdrop-blur-md px-2.5 py-1 rounded-xl shadow-xs border border-emerald-200">
